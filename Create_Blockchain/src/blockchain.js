    ...
    submitStar(address, message, signature, star) {
        ...
        return new Promise(async (resolve, reject) => {
            ...
            if (currentTime - requestTime <= 300) {
                ...
                reject("Verification failed due to more than 5 min lapse time");
            }
        });
    }
    ...
    getBlockByHash(hash) {
        ...
        return new Promise((resolve, reject) => {
            ...
            if (result.length === 1) {
                ...
                reject("No or more than one block found with given hash.")
            }
        });
    }
    ...
    validateChain() {
        ...
        return new Promise(async (resolve, reject) => {
            self.chain.forEach(block => {
                // validate each block
                let blockIsValid = block.validate();
                if (!blockIsValid) {
                    errorLog.push(block);
                }

                // check each block with teh previousBlockHash
                if (block.height > 0) {
                    if (self.chain[i].previousBlockHash != self.chain[block.height - 1].hash) {
                        errorLog.push(block);
                    }
                }
            });
